...
pub fn relabel_sample(path: &Path, empty: bool) -> Result<PathBuf, String> {
    let canon = path
        .canonicalize()
        .map_err(|e| format!("resolve {}: {e}", path.display()))?;
    let empty_root = Path::new(EMPTY_DIR)
    ...
    let full_root = Path::new(FULL_DIR)
        ...
        .map_err(|e| format!("{FULL_DIR}: {e}"))?;
    if !(canon.starts_with(&empty_root) || canon.starts_with(&full_root)) {
        return Err("refusing to move outside dataset folders".into());
    }
    let dest_dir = Path::new(if empty { EMPTY_DIR } else { FULL_DIR });
    ...
    fs::rename(&canon, &dest).map_err(|e| format!("move {}: {e}", canon.display()))?;
    Ok(dest)
}
